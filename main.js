...
function applyContent(data) {
    ...
    elements.forEach(el => {
        ...
        // 1. Get main content value if key exists
        if (keyPath) {
            value = getValueByPath(data, keyPath);
        }

        // 2. Handle HREF updates (independent of main value)
        if (el.tagName === 'A' && el.dataset.contentHref) {
        ...
        // 3. If no main value, we are done
